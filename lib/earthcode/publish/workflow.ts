...
export const getExistingWorkflow = async (
  token: string,
  id: string,
  branch: string
): Promise<{ sha: string; workflow: EarthCODEWorkflow }> => {
  const { sha, content } = await getFile(
    token,
    getWorkflowRecordPath(id),
    branch
  );
...
export const publishWorkflow = async (
  ...
  if (await workflowsExists(token, schema.id)) {
    ...
    const { sha, workflow } = await getExistingWorkflow(
      token,
      schema.id,
      branch
    );
    ...
    workflow.links = [
      ...workflow.links,
      ...experiments.map((e) => getExperimentLink(e)),
    ];

    // Update the workflow file
    await updateFile(
      token,
      getWorkflowRecordPath(schema.id),
      branch,
      sha,
      "Added experiments",
      workflow

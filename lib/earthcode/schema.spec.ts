...
describe("Test EarthCODE Schema Conversion", () => {
    ...
    it("should create a product scheme", () => {
        const product = createProductCollection('worldcereal-maize-detection-product', 'WorldCereal Crop Extent - Belgium', 'Results for batch job cdse-j-25020410530548a7aef81c62faebd127', testProject, jobResults as any);
        expect(product).toEqual(productCollection)
    });

    it("should create a workflow scheme", () => {
        const workflow = createWorkflowCollection('worldcereal-maize-detection-workflow', 'ESA worldcereal global maize detector', 'A maize detection algorithm', testProject, 'https://raw.githubusercontent.com/WorldCereal/worldcereal-classification/refs/tags/worldcereal_crop_type_v1.0.0/src/worldcereal/udp/worldcereal_crop_type.json', ['foobar']);
    ...
    it("should create an experiment scheme", () => {
        ...
        const experiment = createExperimentCollection('worldcereal-maize-detection-experiment', 'ESA worldcereal global maize detector for Belgium', 'A test experiment', 'CC-BY-SA-4.0', testProject, workflow, product);

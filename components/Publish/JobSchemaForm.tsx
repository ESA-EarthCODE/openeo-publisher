...
export const JobSchemaForm = ({schema, projects, themes, onFormChange}: JobSchemaFormProps) => {
    return (
        <div key={`job_form_${schema.job.id}_${schema.type}`}
            ...
            }
            {
                schema.type === SchemaType.WORKFLOW && (
                    <WorkflowForm schema={schema as WorkflowInfo} projects={projects} themes={themes} onFormChange={onFormChange}/>
                )
            }
            {
                schema.type === SchemaType.EXPERIMENT && (
                    <ExperimentForm schema={schema as ExperimentInfo} projects={projects} themes={themes} onFormChange={onFormChange}/>
                )
            }
        </div>
    );
